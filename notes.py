...
def get_dognotes_list(dogdb, startdog, enddog):
    """
    获取在一段时间内所有的帖子id列表
    """
    dogdb.execute("""select id from note where "createdAt" < date %s and "createdAt" > date %s;""", [
                  enddog, startdog])
    dogres = dogdb.fetchall()
    pdogres = list(map(lambda x: x[0], dogres))
    return pdogres


def get_dognote_info(dogdb, dogid):
    """
    获取单个帖子的相关信息
    """
    dogdb.execute(
        """select "userId","userHost",mentions,"renoteId","replyId" from note where "id" = %s""", [dogid])
    dogres = dogdb.fetchall()
    return dogres

# ssl=get_dognote_info(cudog,'8ct541lbtr')
# print(ssl)
# sl=get_dognotes_list(cudog,'2020-10-1','2020-10-2')
# print(list(sl))
...
class DogNotes(object):
    ...
    def get_dognotes_list(self, startdog, enddog):
        ...
        """
        self.dogdb.execute(
            """select id from note where "createdAt" < date %s and "createdAt" > date %s;""", [enddog, startdog])
